...
export function composeUserMessage(
  ...
  if (ctx.hypothesis) {
    ...
    if (ctx.hypothesis.differentials) {
      if (Array.isArray(ctx.hypothesis.differentials)) {
        // Handle array format (legacy)
        parts.push(
          `- Differentials: ${ctx.hypothesis.differentials.map((d: any) => `${d.code}:${d.confidence}`).join(", ")}`,
        )
      } else if (typeof ctx.hypothesis.differentials === "object") {
        // Handle object format (current)
        const diff = ctx.hypothesis.differentials
        if (diff.hypothesis && diff.confidence_score) {
          parts.push(`- Differentials: ${diff.hypothesis} (confidence: ${diff.confidence_score}%)`)
        } else {
          parts.push(`- Differentials: ${JSON.stringify(diff)}`)
        }
      }
    } else {
      parts.push(`- Differentials: n/a`)
    }
  ...
  if (ctx.logs?.length) {
    ...
    ctx.logs.forEach((l: any) => {
      parts.push(
        `- ${l.performed_at}: pain=${l.pain ?? "?"}, rpe=${l.rpe ?? "?"}${l.notes ? `, notes="${String(l.notes).slice(0, 80)}"` : ""}`,
      )
    })
  }
